...
- **Telegram-уведомления**: Отправляет лог в Telegram с указанием имени сервера при успешном обновлении или ошибке (опционально).
...
Скрипт поддерживает отправку уведомлений в Telegram при успешном обновлении или ошибке, включая имя сервера для идентификации. Уведомления отключены по умолчанию и включаются через настройки.
...
- **При успехе**: Отправляется лог (последние 50 строк) с именем сервера при достижении сообщения «Обновление успешно завершено».
- **При ошибке**: Отправляется лог (последние 50 строк) с именем сервера при любом сбое (выход с кодом 1).
- **Другие случаи**: Уведомления не отправляются (например, если обновление не требуется).
...
9. Отправляет уведомления в Telegram (если включено) с именем сервера при успехе или ошибке.
10. Записывает все действия в `/var/log/adguardhome-update.log`.

### Пример вывода
```
[server1] Проверка наличия утилит...
[server1] Все необходимые утилиты установлены.
[server1] Проверка места на диске...
[server1] Место на диске в /tmp доступно.
[server1] Очистка старых резервных копий...
[server1] Очистка резервных копий завершена. Хранится 2 копий.
[server1] Директории созданы и доступны для записи.
[server1] Проверка разрешения DNS для github.com и static.adguard.com...
[server1] DNS работает корректно.
[server1] Проверка сетевой доступности...
[server1] Сетевое соединение доступно.
[server1] Получение текущей версии AdGuard Home...
[server1] Текущая версия: v0.107.61
[server1] Получение последней версии с GitHub...
[server1] Последняя версия: v0.107.61
...
Скрипт проверяет размер лог-файла `/var/log/adguardhome-update.log` и пересоздаёт его, если он превышает 1 МБ. Это предотвращает бесконечный рост логов при ежедневном запуске. Если лог-файл или его директория отсутствуют, они создаются автоматически.
