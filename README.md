...
- **Telegram-уведомления**: Отправляет краткое сообщение с эмодзи при успешном обновлении (✅), подробный лог текущей операции в формате кода при ошибке (❌) или по команде `--telegram` (ℹ️).
...
- **С параметром `--telegram`**: Отправляется лог текущей операции в формате кода с ℹ️: «ℹ️ Результат на [имя_сервера]: <лог>».
- **Другие случаи**: Уведомления не отправляются, если `ENABLE_TELEGRAM="false"` и `--telegram` не указан.

### Пример Telegram-уведомлений
- **Успех**:
  ```
  ✅ AdGuard на [server1] обновлён до версии v0.107.62
  ```
- **Ошибка**:
  ```
  ❌ Ошибка на [server1]:
  ...
  [2025-04-29 15:42:00] [server1] Проверка доступной версии AdGuard Home...
  [2025-04-29 15:42:00] [server1] Ошибка: Не удалось получить информацию о последней версии...
  [2025-04-29 15:42:00] [server1] Проверка версии: [ОШИБКА]
...
- **С `--telegram`**:
  ```
  ℹ️ Результат на [server1]:
  ```
  [2025-04-29 15:42:00] [server1] Запуск скрипта обновления AdGuard Home...
  [2025-04-29 15:42:00] [server1] Проверка доступной версии AdGuard Home...
  [2025-04-29 15:42:00] [server1] Проверка версии: [ОК (v0.107.61)]
  [2025-04-29 15:42:00] [server1] Проверка доступной версии: v0.107.61, совпадает с текущей [ОК]
  ```
...
9. Отправляет уведомления в Telegram:
   ...
   - Полный лог текущей операции в формате кода с ❌ при ошибке или ℹ️ с `--telegram`.
...
- **Без обновления**:
  ...
  [server1] Проверка доступной версии AdGuard Home...
  [server1] Проверка версии: [ОК (v0.107.61)]
  [server1] Проверка доступной версии: v0.107.61, совпадает с текущей [ОК]
  ```
- **С обновлением или ошибкой**:
  ```
  [server1] Запуск скрипта обновления AdGuard Home...
  [server1] Проверка доступной версии AdGuard Home...
  [server1] Проверка версии: [ОК (v0.107.62)]
  [server1] Доступна новая версия v0.107.62, текущая v0.107.61, начинаем обновление...
  ...
  ...
  [server1] Новая установленная версия: v0.107.62
  [server1] Проверка новой версии: [ОК]
  [server1] +----------------------------------+--------+
  [server1] | Операция                         | Статус |
  [server1] +----------------------------------+--------+
  [server1] | Проверка версии                  | [ОК]   |
  [server1] | Проверка диска                   | [ОК]   |
