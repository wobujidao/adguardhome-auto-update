...
3. Отредактируйте скрипт:
   ...
   SERVER_NAME="server.example.com"  # Опционально, если не указано, используется системное имя хоста
   ```
   Сохраните изменения.
...
- **Успех**:
  ```
  ✅ AdGuard на server.example.com обновлён до версии v0.107.62
  ```
- **Ошибка**:
  ```
  ❌ Ошибка на server.example.com:
  [2025-04-29 17:26:58] [server.example.com] Запуск скрипта обновления AdGuard Home...
  [2025-04-29 17:26:58] [server.example.com] Проверка доступной версии AdGuard Home...
  [2025-04-29 17:26:58] [server.example.com] Ошибка: Не удалось получить информацию о последней версии...
  [2025-04-29 17:26:58] [server.example.com] Проверка версии: [ОШИБКА]
  [2025-04-29 17:26:58] [server.example.com] +----------------------------------+--------+
  [2025-04-29 17:26:58] [server.example.com] | Операция                         | Статус |
  [2025-04-29 17:26:58] [server.example.com] +----------------------------------+--------+
  [2025-04-29 17:26:58] [server.example.com] | Проверка версии                  | [ОШИБКА]|
  [2025-04-29 17:26:58] [server.example.com] +----------------------------------+--------+
  ```
- **С `--telegram`**:
  ```
  ℹ️ Проверка обновления AdGuard Home на server.example.com: версия v0.107.61, установлена v0.107.61, обновление не требуется
  ```
...
- **Без обновления**:
  ```
  [server.example.com] Запуск скрипта обновления AdGuard Home...
  [server.example.com] Проверка доступной версии AdGuard Home...
  [server.example.com] Проверка версии: [ОК (v0.107.61)]
  [server.example.com] Проверка доступной версии: v0.107.61, совпадает с текущей [ОК]
  ```
- **С обновлением или ошибкой**:
  ```
  [server.example.com] Запуск скрипта обновления AdGuard Home...
  [server.example.com] Проверка доступной версии AdGuard Home...
  [server.example.com] Проверка версии: [ОК (v0.107.62)]
  [server.example.com] Доступна новая версия v0.107.62, текущая v0.107.61, начинаем обновление...
  [server.example.com] Проверка места на диске...
  [server.example.com] Проверка диска: [ОК]
  [server.example.com] Очистка старых резервных копий...
  [server.example.com] Очистка копий: [ОК (2 копий)]
  ...
  [server.example.com] Новая установленная версия: v0.107.62
  [server.example.com] Проверка новой версии: [ОК]
  [server.example.com] +----------------------------------+--------+
  [server.example.com] | Операция                         | Статус |
  [server.example.com] +----------------------------------+--------+
  [server.example.com] | Проверка версии                  | [ОК]   |
  [server.example.com] | Проверка диска                   | [ОК]   |
  [server.example.com] | Очистка копий                    | [ОК]   |
  ...
  [server.example.com] +----------------------------------+--------+
  ```
