...
- **Управление логами**: Пересоздаёт лог-файл, если он превышает 1 МБ, и добавляет статус `[ОК]` или `[ОШИБКА]` для операций.
...
- **Telegram-уведомления**: Отправляет лог текущей операции с именем сервера при успешном обновлении, ошибке или по команде `--telegram` (опционально).
...
Скрипт поддерживает отправку уведомлений в Telegram с логом текущей операции, включая имя сервера для идентификации. Уведомления отключены по умолчанию и включаются через настройки или параметр `--telegram`.
...
- **При успехе**: Отправляется лог текущей операции с именем сервера при достижении сообщения «Обновление успешно завершено».
- **При ошибке**: Отправляется лог текущей операции с именем сервера при любом сбое (выход с кодом 1).
- **С параметром `--telegram`**: Отправляется лог текущей операции с именем сервера при любом завершении скрипта (успех, ошибка, отсутствие обновления).
...
9. Отправляет уведомления в Telegram (если включено или указан `--telegram`) с логом текущей операции и именем сервера.
10. Записывает все действия в `/var/log/adguardhome-update.log` с добавлением статуса `[ОК]` или `[ОШИБКА]` и пустой строки между запусками.
...
3. Для принудительной отправки уведомлений в Telegram добавьте параметр `--telegram`:
   ```bash
...
Скрипт записывает действия в `/var/log/adguardhome-update.log` с добавлением статуса `[ОК]` или `[ОШИБКА]` для каждой операции и пустой строки между запусками для удобства чтения. Лог текущей операции временно сохраняется в `/tmp/adguardhome-current.log` для Telegram-уведомлений. Если основной лог превышает 1 МБ, он пересоздаётся. Если лог-файл или его директория отсутствуют, они создаются автоматически.
